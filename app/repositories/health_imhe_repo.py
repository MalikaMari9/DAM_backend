...
def _split_filters_by_source(params: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    raw_filters: dict[str, Any] | None = dict(base_filters)
    ...
    *,
    ...
    if raw_filters is not None:
        ...
        pipeline.extend(
            [
                {"$sort": key_sort},
                {"$group": {"_id": key_group, "doc": {"$first": "$$ROOT"}}},
                {"$replaceRoot": {"newRoot": "$doc"}},
                {"$project": {"__source_rank": 0}},
            ]
        )
...
def list_imhe(params: dict[str, Any], limit: int, offset: int):
    ...
    col, pipeline = _start_pipeline_for_sources(raw_filters, pred_filters, project={"_id": 0})
    if col is None or pipeline is None:
        return 0, []
    items = list(col.aggregate([*pipeline, {"$skip": int(offset)}, {"$limit": int(limit)}]))
...
def list_locations(params: dict[str, Any]):
    ...
    return [row["_id"] for row in col.aggregate(pipeline) if row.get("_id")]
...
def country_summary(params: dict[str, Any]):
    year = params.get("year")
    if _is_count_like_metric(params) and isinstance(year, (int, float)):
        raw_rows = _country_summary_from_collection(get_imhe_collection(), params)
        if int(year) < IMHE_PRED_START_YEAR:
            return raw_rows
        pred_params = _pred_only_params(params)
        if pred_params is None:
            return raw_rows
        pred_rows = _country_summary_from_collection(get_imhe_pred_collection(), pred_params)
...
def _country_summary_from_collection(col, params: dict[str, Any]):
    filters = _build_filters(params)
    aggregate_op = "$sum" if _is_count_like_metric(params) else "$avg"
    pipeline = [
        {"$match": filters},
...
def _country_year_totals_from_collection(col, params: dict[str, Any]):
    filters = _build_filters(params)
    pipeline = [
        {"$match": filters},
        {
            "$group": {
                "_id": {"country": "$location_name", "year": "$year"},
                "value": {"$sum": "$val"},
            }
        },
        {
...
def _pred_only_params(params: dict[str, Any]) -> dict[str, Any] | None:
    pred_params = dict(params)
    _raw_year_filter, pred_year_filter = _split_year_filter(params.get("year"))
    if pred_year_filter is None:
        return None
...
def trend_by_year(params: dict[str, Any]):
    if _is_count_like_metric(params):
        raw_rows = _trend_by_year_from_collection(get_imhe_collection(), params)
        pred_params = _pred_only_params(params)
        if pred_params is None:
            return raw_rows

        country_name = params.get("location_name")
        if country_name:
            pred_rows = _trend_by_year_from_collection(get_imhe_pred_collection(), pred_params)
        ...
        raw_country_year = _country_year_totals_from_collection(get_imhe_collection(), params)
        pred_country_year = _country_year_totals_from_collection(get_imhe_pred_collection(), pred_params)
        merged_country_year: dict[tuple[str, int], float] = {}
        for row in raw_country_year:
            country = row.get("country")
            year = row.get("year")
            if country and year is not None:
                merged_country_year[(country, int(year))] = float(row.get("value") or 0)
        for row in pred_country_year:
            ...
            if country and year is not None:
                ...
                if key not in merged_country_year:
                    merged_country_year[key] = float(row.get("value") or 0)

        totals_by_year: dict[int, float] = {}
        for (_country, year), value in merged_country_year.items():
            totals_by_year[year] = totals_by_year.get(year, 0.0) + value
        return [{"year": year, "value": totals_by_year[year]} for year in sorted(totals_by_year)]
